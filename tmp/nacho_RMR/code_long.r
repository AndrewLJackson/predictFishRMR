...
pars <- c(0.83, 0.04, 1.28, 0.53, 5.945)
...
axis(2, las = 1, at = log10(c(0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000)), 
  labels = c(expression("10"^-7*""), expression("10"^-6*""), expression("10"^-5*""), expression("10"^-4*""), 
    expression("10"^-3*""), expression("10"^-2*""), expression("10"^-1*""), expression("1"), expression("10")))    
...
m <- size_range
plot(m, RMR_meso_20/1000, type = "n", ylim = c(0,420), xlim = c(0,2000), las = 1, ylab = "", xlab = "", col = "red", xaxs = "i", yaxs = "i", xaxt = "n")
...
text(1200, 300, "Mesotherms", cex = 1.2)
