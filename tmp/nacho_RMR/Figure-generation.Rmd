...
kpars <- c(0.00174, -0.63180) # exclude
# kpars <- c(0.00201, -0.6166) # include
...
plot(size_range, elevation_values, type = "l", col = "black", yaxs = "i", las = 1, xlim = c(0,2000), ylim = c(0,15), xaxs = "i", ylab = "", xaxt = "n", xlab = "", lwd = 1.5)
axis(1, at = c(1,500,1000,2000), labels = c("1 Kg", "500 Kg", "1Ton", "2 Ton"))
...
lines(dd_subset[dd_subset$type == "meso",]$mass, dd_subset[dd_subset$type == "meso",]$Ta_crit, lwd = 1.5, cex = 0.8, col = "black", lty = 2)
...
world_ta2 <- raster("future_min.nc")
x_max <- maxAmbientTemp(m, meso, pars)
...
world_ta2 <- raster("future_max.nc")
x_max <- maxAmbientTemp(m, meso, pars)
values(world_ta2) <- ifelse(values(world_ta2)>x_max, NA, values(world_ta2) )
Ta <- Tm <- world_ta2
values(Tm) <- bodyTempLambertRaster(x = values(Ta), m = m, meso = meso, 
...
plot(RMR_old ~ RMR_new, type = "l", main = paste0("mass = ", m))
