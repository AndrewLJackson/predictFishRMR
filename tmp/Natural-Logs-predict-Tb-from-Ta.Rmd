...
# for (i in 1:length(r_files)){source(r_files[i])}
# omega <- 10^5.954
...
$$\log_{10}(RMR) = \alpha\ln(m) + \beta T_m + \gamma(S)$$

$$RMR = e^{(\alpha\ln(m) + \beta T_m + \gamma(S))}$$
$$RMR = m^\alpha e^{\beta T_m}e^{\gamma(S)}$$

Heat production via metabolism is then calculated as

$$\dot{T}_0 = \frac{RMR}{M*\omega}$$
where $\omega = 10^5.945$ (CHECK REFERENCE).
...
$$T_a = T_m - \frac{e^{(\alpha\log(m) + \beta T_m + \gamma(S))}}{m*\omega*k}$$
where:
...
- $\gamma(S)$ is the intercept which takes different values depending on the thermal strategy. In our models, this defines either ecotherms or mesotherms. In the statistical model, a single intercept is fitted with an additional additive ceofficient for mesotherms such that $\gamma(\text{mesotherm}) = \gamma(\text{ectotherm}) + \psi$
- $\omega$ is the the coefficient of mass that converts $RMR$ to $\dot{T}_0$, currently $\omega = 10^{5.945}$
- $k$ is the rate of heat transfer and is estimated via $k = 0.00161 * m^{-0.62}$



The symobolic solver available at [wolfram](https://www.wolframalpha.com/input?i2d=true&i=solve+for+B%5C%2844%29+A+%3D+B+-+%5C%2840%29Divide%5BPower%5B10%2C%5C%2840%29α*Log10%5Bm%5D%2Bβ*B%2Bγ%2Bω%5C%2841%29%5D%2Cm*Power%5B10%2Cψ%5D*k%5D%5C%2841%29++) yields the following solution where we introduce $B = T_m$ (B for body) and $A = T_a$ (A for ambient) for clarity for the solver.

Natural log form

`solve for B, A = B - (Divide[Exp[α*Ln[m]+β*B+γ],m*ω*k])`

$$T_m = T_a - W_n \left (-\frac{e^{(\beta T_a + \gamma(S))}(m^{\alpha - 1})\beta}{\omega k}\right) \frac{1}{\beta} $$

## Define parameters and functions for this model

Define the parameters of the model taken from the PGLS regression model. Note that here is where $\gamma(S)$ has to become a general intercept $\gamma$ which references ectotherms with an additive term applied for when the thermal strategy is endothermy $\gamma + \psi$
...
# 5 = omega, the coefficient of mass for converting RMR to T0
# pars <- c(0.8161589, 0.0505157, 0.8700325, 0.8358592, 10^5.945)
# pars <- c(0.8700325, 0.8161589, 0.0505157 , 0.8358592, 10^5.945)
# pars <- c(2.9468, 0.8271, 0.0805, 1.2333)
data("fittedModelRMR")
